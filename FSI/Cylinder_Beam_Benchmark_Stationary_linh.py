...
###########################
# Benchmark: Cylinder & Flexible beam assembly in a channel (time-invariant)
###########################
"""
The goal is to solve a steady state Navier-Stokes as a first step before implementing the full NS.
This is treating a stationary FSI with harmonic and linear-elastic mesh motion
following the benchmark description available at and the formulation in Problem 7.22:
...
if __name__ == "__main__":
    ...
    output_dir = "results/results_Cylbeam_stationary_benchmark"
    ...
    # md.add_filtered_fem_variable("p_solid", mfp_solid,BEAM)   # Pressure of the solid (beam)
    ...
    # Mesh motion
    md.add_macro("sigma_mesh(u)", "Grad(u)")   # Harmonic mesh

    # Continuity
    md.add_macro("Continuity(u, v)", "J(u) * Trace(Grad(v) * Inv(F(u)))")

    # Fluid momentum (No body force: f = 0)
    md.add_nonlinear_term(mim, "rho_f*J(u_fluid)*((Inv(F(u_fluid))*v_fluid).Grad_v_fluid).(Test_v_fluid)", FLUID)                              # Convective term
    ...
    # Solid Momentum (No body force: f = 0)
    ...
    #### To check which test function to use
    md.add_nonlinear_term(mim, "-((J(u_fluid)*(sigma_f_p(p_fluid) + sigma_f_vu(u_fluid, v_fluid))*((Inv(F(u_fluid)))'))*Normal).Test_u_solid", INTERFACE)   # Fluid traction term
    ...
    # Equal displacement at the interface
    ...
    md.add_Dirichlet_condition_with_multipliers(mim, "v_fluid", mfv_fluid, INTERFACE, "V_noslip")
    ...
    V_inlet_expr = "[4*1.5*X(2)*(H-X(2))/(H*H), 0]"
    ...
    # Only export what was actually solved
    mfv_fluid.export_to_vtu(f"{output_dir}/fluid_results.vtu",
                            mfu_fluid, u_f_full, "Displacement",  
